...
// だけで判定してはいけない：実データで確認済みの通り、横浜銀行の分類（当初「楽天カード」
// という名前だったため、みずほ銀行の楽天カードえみ明細と同じものと誤認し、2026-08-29
// 時点では分類名だけで一致判定して誤って対象に含めていた）は、実際にはクレジットカード
// ではなく楽天証券への定期積立（横浜銀行から引き落とし）で、journal_classification_map
// 上も「投資」に対応付けられており、他のカード引き落とし行の「－」とは扱いが違う、
// みずほ銀行の楽天カードえみ（恵美様名義の実際のカード）とは無関係の別物のため対象に
// 含めてはいけない（2026-08-30、本人の指摘を受けて紛らわしい「楽天カード」という
// 分類名を実態に合わせ「楽天証券積立」に改名し、この誤解が今後起きないようにした）。
