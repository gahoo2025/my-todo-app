// タスクの開始・終了スケジュール入力欄（終日/時間指定の切替付き）
export default function ScheduleFields({ value, onChange }) {
  const { enabled, allDay, startDate, startTime, endDate, endTime } = value
    ...
    <div className="rounded-xl border border-gray-100 bg-gray-50 p-3 space-y-3">
      <div className="flex items-center justify-between">
        <label className="flex items-center gap-2 cursor-pointer">
          ...
          />
          <span className="text-sm font-medium text-gray-600">スケジュールを設定</span>
        </label>
        {enabled && (
