...
### 未仕訳の学習ルール（journal_custom_rules）

「未仕訳」タブで1件を仕訳確定する際、既存535件ルールで一切マッチしなかった摘要（PayPay等、あえて毎回確認が必要な摘要は対象外）に限り、「次回から自動仕訳する」にチェックを入れると、その摘要パターンが学習ルールとして保存され、次回以降の明細インポートで自動仕訳されます（2026-09-20追加）。利用するにはSupabaseで以下のテーブルを作成してください（**2026-09-20時点、本番プロジェクトには適用済み**）。

```sql
create table journal_custom_rules (
...
create unique index journal_custom_rules_unique_pattern
  on journal_custom_rules (user_id, institution, card_holder, pattern);

alter table journal_custom_rules enable row level security;

create policy "Users can manage their own custom rules"
  on journal_custom_rules for all
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);
```

`pattern`は摘要の生文字列（マッチング時に`normalizeText`で正規化して比較）。同じ（取引先・カード名義・摘要パターン）はupsertで上書きされるため、誤って登録した場合はもう一度チェックを入れて仕訳し直せば訂正できる。
